...
As we recall from the introduction, our motivation for this thesis is in the improvement of medical diagnosis with the help of machine learning. 
We talked about existing methods for screening developed to work with datasets retried from the medical professionals. 
...
Medical images can be classified in to multiple categories, 
...
\label{cha:CAD}
In the previous section, we gave a summary of the medical procedure associated with a colonoscopy. Though in the twenty-first century the focus has shifted to the quest to automatise the process.
...
Despite the numerous papers published on the subject, there are still challenges that still need more research. 
...
When it comes to network training,  Tajbakhsh et al. demonstrated how fine-tuning a pretraind CNN in a layer-wise manner leads to incremental performance improvement in medical images~\cite{DBLP:journals/corr/TajbakhshSGHKGL17}.

Pogorelov et al. In 2016 presented a complete end-to-end multimedia system for tackling automatic analysis of GI tract videos. The proposed system includes a pipeline ranging from data collecting, processing and data analysis, to visualisation~\cite{Pogorelov2017}. 

Pogorelov et al. also recently published a paper on the generalisation of data, for the purpose of using open datasets for training CAD systems~\cite{pogorelov2018deep}. In this paper, they presented hand-crafted and deep learning-based methods for detection of polyps in videos from colonoscopies. In this paper the worked to achieve real-world comparability by using challenging datasets captured with different kinds of hardware. In addition to this, they used imbalanced datasets and as little as possible training data.
Their best model, a GAN for handcrafted features, reached a detection specificity of 94\% and an accuracy of 90.9\%, done with only 356 training samples.
...
\todo{Future works in this paper is the basis for this thesis}
...
Projects like Aplha GO and OpenAI Five shows that, given the right amount and type of data, our machine learning algorithms can solve the same problems humans solve, given both the right training environment and enough time.
...
Figure \ref{fig:neuron}\footnote{From \url{https://commons.wikimedia.org/wiki/File:Neuron.svg}} shows how a human neuron looks like, and in which direction the signal goes. Each neuron is connected to multiple other neurons by connecting the dendrites to other neighbouring neurons forming a pathway for the electrical signals to flow. 
...
Global pooling has the effect of transforming the network from 3 dimensions to 1 dimension, making it ideal right before fully connected layers at the end of longer networks.
