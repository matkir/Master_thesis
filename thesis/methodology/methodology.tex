...
We first get a birds-eye view of the objective of this thesis, looking at the hypothesises behind this thesis, and take a look into how we can test the proposed hypothesises.
We look at the language and packages used in the creation of this thesis. We go in depth into the reasoning behind why we chose the tools and packages that became the foundation of the programs. 
...
To help solve these predicaments look back at the two hypotheises presented in section~\ref{cha:problemstatement}:

Hypothesis~\ref{hyp:a} stated that sparse information in the images made it harder to classify the images correctly. Figure \ref{fig:Saliencymasks} from Hicks's paper shows the case where the black areas with sparse information affect the classification, based only on the black \todo{rewrite}

Hypothesis~\ref{hyp:b} stated that dataset spessific artefacts creates false positives and negatives. This is clearly shown in \ref{fig:sal2}, where the classification is clearly affected by the green square in the image.
...
We can see by removing thing that things \todo{fix this}
...
We use this as a basis for our work, and look into how we want to solve the same problems encountered. 
